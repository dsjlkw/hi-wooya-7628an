...
curl --retry 5 -L "https://downloads.openwrt.org/releases/$(printf "%s" "$REPO_BRANCH" | cut -c 2-)/targets/ramips/mt76x8/config.buildinfo" > .config
sed -e '/^CONFIG_TARGET_DEVICE_/d' -e '/CONFIG_TARGET_MULTI_PROFILE=y/d' -i .config
